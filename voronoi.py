...
def mosaic(filename, n_samples):
    ...
    for i, reg in enumerate(regions):
        ...
        sample_pt = points[i]
        sample_color = img[height - 1 - sample_pt[1], sample_pt[0]]
        plt.fill(*zip(*polygon), color=sample_color/256.)
...
def main():
    parser = argparse.ArgumentParser(description="turn image into a mosaic using random sampling and voronoi diagrams")
    parser.add_argument('filename', type=str, nargs=1)
    parser.add_argument('n_samples', type=int, nargs=1)
    args = parser.parse_args()
    mosaic(args.filename[0], args.n_samples[0])
